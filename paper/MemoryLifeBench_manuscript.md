...
LLM-judge rescoring (GPT-4o judges each prediction against the reference, independent of exact wording; exact prompt in Appendix C.3) of the same 725 predictions found 22.1% (160/725) were marked wrong by EM but judged substantively correct. Under the judge metric, LoCoMo scores rise approximately 3.6–4.4x across every policy (e.g. `ours_utility`: EM 0.0917 to judge 0.3500, a 3.8x rise), and the relative ranking shifts further in `ours_utility`'s favor — it edges out both `lru` (0.3417) and the `no_forget` ceiling itself (0.3333).

**Update (bootstrap significance on the judge metric, closing a Limitations gap).** Applying the same paired bootstrap treatment as Section 6.6 (10,000 resamples, same-question resampling) to the judge metric confirms the EM/F1 story rather than overturning it: `ours_utility` still significantly beats `fifo` (+0.1671, 95% CI [+0.0833, +0.2500], p<0.001) and `ours` (+0.1505, 95% CI [+0.0750, +0.2333], p<0.001), and remains statistically tied with `no_forget` (p=0.269) and `lru` (p=0.421). The judge metric raises absolute scores but does not change which differences are and are not significant — this is no longer a raw mean cited without significance testing.
...
- **The `ours_utility` vs. `lru` comparison is not statistically significant** at n=120 real LLM-scored questions (Section 6.6), even though the free-diagnostic proxy and the mechanistic AUC analysis both point the same direction. This holds under the judge metric too (Section 6.8, p=0.421), not only EM/F1.
- **The Importance head is a heuristic, not learned** — no ground-truth importance label exists in the dataset schema.
- **The residual gap between `ours` and `lru`/`fifo` at every TTL quantile** (Section 6.4) motivated a working hypothesis — that LoCoMo's QA evidence is itself recency-skewed, mechanically favoring recency-based selection — which we have since tested directly (unpaired bootstrap over all 2,536 LoCoMo memories' evidence-linked vs. non-evidence ages) and found **significantly refuted, in the reversed direction**: evidence-linked memories are on average *older* than non-evidence memories (104.2 vs. 97.3 days, 95% CI on the gap [-12.78, -1.02] days, p=0.01). The residual gap is therefore not explained by recency-skew; it strengthens the Section 6.5 structural (ranking-mechanism) explanation instead, since `lru` wins despite working against, not with, this mild anti-recency skew. (Full analysis: `scripts/analyze_evidence_recency_skew.py`, `results/tables/week7_evidence_recency_skew.md`.)
...
- **No human validation of extracted labels has been performed end-to-end**; all labels are programmatically derived. A stratified 45-record human-review sample (source dialogue text joined in for LoCoMo/synthetic records) has been prepared (`scripts/sample_for_human_validation.py`, `results/tables/week7_human_validation_sample.csv`) but not yet completed by a rater, so this remains open rather than closed. Separately, a full-coverage (not sampled) automated audit recomputing every record's `duration_days` from raw timestamps found 4.63% (470/10,152) mismatches, all in LongMemEval; 449 of those match a floor-clamp pattern or are within 1 day, and 21 (0.21% of the full dataset) have a genuine, unexplained gap up to 53.2 days that reverse-engineering the (unreleased) extraction pipeline would be needed to resolve — reported here rather than rounded away. (Full audit, including a disclosed process note about a bug caught and fixed in the audit script itself before this number was reported: `scripts/audit_label_consistency.py`, `results/tables/week7_label_consistency_audit.md`.)
- **LLM-judge scoring** (Section 6.8) is itself imperfect — see the flagged judge-error example in Section 6.9 — though its results are now confirmed by the same bootstrap significance testing as the EM/F1 headline claims (Section 6.8), not just reported as raw means.
